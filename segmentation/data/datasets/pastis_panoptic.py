...
class PastisPanoptic(tdata.Dataset):
    def __init__(
        self,
        folder,
        norm=True,
        ...
        Args:
            ...
            sats (list): defines the satellites to use (only Sentinel-2 is available
                in v1.0)
        """
        super(PastisPanoptic, self).__init__()
        self.folder = folder
        self.norm = norm
        ...
        if isinstance(mono_date,str):
            self.mono_date = datetime(*map(int, mono_date.split("-")))
        elif isinstance(mono_date,int):
        ...
        self.instance_map_ids =  self.meta_patch['N_Parcel']
